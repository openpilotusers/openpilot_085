...
public:
  MdpsHarnessToggle() : ToggleControl("MDPS Harness 사용", "MDPS 하네스 사용시 기능을 켜십시", "../assets/offroad/icon_shell.png", Params().getBool("MdpsHarness")) {
    QObject::connect(this, &MdpsHarnessToggle::toggleFlipped, [=](int state) {
      char value = state ? '1' : '0';
      Params().put("MdpsHarness", &value, 1);
    });
  }
};
