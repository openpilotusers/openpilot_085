...
class CarInterfaceBase():
  ...
  def create_common_events(self, cs_out, extra_gears=[], gas_resume_speed=-1, pcm_enable=True):  # pylint: disable=dangerous-default-value
    ...
    if cs_out.gearShifter != GearShifter.drive and cs_out.gearShifter not in extra_gears and cs_out.steeringAngleDeg < 90:
      self.user_disable_timer += 1
      if self.user_disable_timer > 50:
        events.add(EventName.wrongGear)
        self.user_disable = True
    elif cs_out.gearShifter != GearShifter.drive and cs_out.gearShifter not in extra_gears and self.user_disable:
      self.user_disable_timer += 1
      if self.user_disable_timer > 50:
        events.add(EventName.wrongGear)
    else:
