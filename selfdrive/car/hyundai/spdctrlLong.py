...
class SpdctrlLong(SpdController):
    ...
    def update_lead(self, sm, CS, dRel, yRel, vRel, CC):
        ...
        # 선행차량이 멀리 있는 상태에서 감속 조건
            ...
            #     lead_wait_cmd, lead_set_speed = self.get_tm_speed(CS, CC, max(25, 70+(lead_objspd*2)), -1)
            elif 65 > CS.clu_Vanz > 30 and lead_objspd < -3 and int(CS.clu_Vanz) <= CC.setspeed and int(CS.clu_Vanz) >= dRel*0.85 and 1 < dRel < 149:
                self.seq_step_debug = "SS>VS,60>v>30,-1"
                lead_wait_cmd, lead_set_speed = self.get_tm_speed(CS, CC, max(20, 220-(abs(lead_objspd**3))), -1)
                ...
                lead_wait_cmd, lead_set_speed = self.get_tm_speed(CS, CC, 20, -5)
            elif lead_objspd == 0 and int(CS.clu_Vanz)+4 <= CC.setspeed and int(CS.clu_Vanz) > 40 and 1 < dRel < 149: # 앞차와 속도 같을 시 현재속도+5으로 크루즈설정속도 유지
